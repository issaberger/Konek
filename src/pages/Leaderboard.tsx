  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#f5f5f0]">
      <header className="bg-white p-4 shadow-sm sticky top-0 z-10">
        <h1 className="text-2xl font-serif font-bold text-[#00209F]">{t('leaderboard')}</h1>
        <p className="text-sm text-gray-500">{t('topStudents')}</p>
      </header>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl overflow-hidden shadow-sm">
            {leaders.map((leader, index) => (
              <div 
                key={leader.id} 
                className={`flex items-center p-4 border-b border-gray-100 last:border-0 ${index < 3 ? 'bg-blue-50/50' : ''}`}
              >
